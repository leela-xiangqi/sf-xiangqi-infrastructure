...
#else
#  define pext(b, m) __pext_u64(b, m)
#endif
...
enum PieceType {
  NO_PIECE_TYPE, SOLDIER, CANNON, CHARIOT, HORSE, ELEPHANT, ADVISOR, GENERAL,
  ALL_PIECES = 0,
  PIECE_TYPE_NB = 8
};
...
enum Piece {
  NO_PIECE,
  R_SOLDIER = 1, R_CANNON, R_CHARIOT, R_HORSE, R_ELEPHANT, R_ADVISOR, R_GENERAL,
  B_SOLDIER = 9, B_CANNON, B_CHARIOT, B_HORSE, B_ELEPHANT, B_ADVISOR, B_GENERAL,
  PIECE_NB = 16
};
...
constexpr Square operator~(Square s) {
  return Square(SQ_A9 - s + ((s % 9) << 1)); // Vertical flip SQ_A0 -> SQ_A9
}

constexpr File operator~(File f) {
  return File(FILE_I - f); // Horizontal flip FILE_A -> FILE_I
}

constexpr Piece operator~(Piece pc) {
  return Piece(pc ^ 8); // Swap color of piece B_ELEPHANT -> W_ELEPHANT
}
...
constexpr bool crossed_river(Color c, Square s) {
  return relative_rank(c, s) > RANK_4;
}

constexpr Direction soldier_push(Color c) {
  return c == RED ? NORTH : SOUTH;
}
